...
- `components/canvas/MobileHostsSection.tsx` — mobile section with quick actions. Mirrors the desktop dialog's missing-`apiKey` recovery: when the workspace API key isn't present in this browser, it shows the "API key unavailable" explanation with a "Create new workspace" action in place of the hosts list and inline install instructions.
